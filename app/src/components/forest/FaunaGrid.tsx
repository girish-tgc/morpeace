...
function FaunaTile({ f, onOpen }: { f: FaunaSpecies; onOpen: (f: FaunaSpecies) => void }) {
  return (
    <button
      ...
      onClick={() => onOpen(f)}
      data-fauna-tile
      className="group relative overflow-hidden rounded-xl aspect-square bg-[#014066]/40 ring-1 ring-sky-cream/10 hover:ring-[#F5EBD0]/45 transition-all duration-500 text-left"
    >
      <img
        src={`${BASE}${f.photos[0]}`}
        alt={f.common}
