...
// Official Google Maps URL API — resolves to the exact pin on both desktop web
// and the mobile Google Maps app. The older /maps/place/Name/@lat,lng form
// deep-links into the mobile app without a place ID and loses the location.
const MAPS_URL =
  'https://www.google.com/maps/search/?api=1&query=17.6105128%2C73.9891524'
...
type ActivityItem = { icon: Icon; title: string; description: string }

const LAND_ACTIVITIES: ActivityItem[] = [
...
const RECREATION_ACTIVITIES: ActivityItem[] = [
...
const SUSTAINABILITY_ITEMS: ActivityItem[] = [
    ...
    >
      ...
      >
        <video
          ...
          autoPlay
          preload="metadata"
          poster={`/media/property/meditation-room-circle.webp`}
          className="absolute inset-0 w-full h-full object-cover"
        >
          <source src={`/photos/meditation-drone-2.mp4`} type="video/mp4" />
        </video>
      ...
      >
        <img
          src={`/photos/mangoes-sunlight.webp`}
      ...
      <section id="celebrations" className="py-20 md:py-28 px-6 md:px-10 scroll-mt-28" style={{ backgroundColor: `${PALETTE.deep}cc` }}>
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14 items-center">
          ...
          >
            <img
              src={`/media/property/sham-kane-renonwed-tabla-player.webp`}
